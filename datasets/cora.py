...
def load_citation_data(args):
    ...
    for i in range(len(names)):
        with open("{}/ind.{}.{}".format(args.citation_root, args.activate_dataset, names[i]), 'rb') as f:
            objects.append(pkl.load(f, encoding='latin1'))

    x, y, tx, ty, allx, ally, graph = tuple(objects)
    test_idx_reorder = parse_index_file("{}/ind.{}.test.index".format(args.citation_root, args.activate_dataset))
    test_idx_range = np.sort(test_idx_reorder)

    if args.activate_dataset == 'citeseer':
